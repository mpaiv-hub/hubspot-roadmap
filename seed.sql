...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'roadmap_comments' AND policyname = 'Allow all'
  ) THEN
    CREATE POLICY "Allow all" ON roadmap_comments FOR ALL USING (true) WITH CHECK (true);
  END IF;
END
$$;

-- 2. Delete all existing data (order matters for FK constraints)
DELETE FROM roadmap_subtasks;
DELETE FROM roadmap_tasks;
DELETE FROM roadmap_phases;
DELETE FROM roadmap_state;
DELETE FROM step_status;
DELETE FROM roadmap_comments;

-- 3. Insert Phases
INSERT INTO roadmap_phases (id, title, subtitle, duration, color, "order", created_at, updated_at) VALUES
...
INSERT INTO roadmap_tasks (id, phase_id, title, description, owner, output, dependency, "order", created_at, updated_at) VALUES
...
INSERT INTO roadmap_subtasks (id, task_id, text, "order", created_at, updated_at) VALUES
